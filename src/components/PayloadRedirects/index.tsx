...
export const PayloadRedirects: React.FC<Props> = async ({ disableNotFound, url }) => {
  ...
  if (redirectItem) {
    ...
    if (typeof redirectItem.to?.reference?.value === 'string') {
      ...
      redirectUrl = `${redirectItem.to?.reference?.relationTo !== 'pages' ? `/${redirectItem.to?.reference?.relationTo}` : ''}/${typeof redirectItem.to?.reference?.value === 'object' ? redirectItem.to?.reference?.value?.slug : ''}`;
    }
